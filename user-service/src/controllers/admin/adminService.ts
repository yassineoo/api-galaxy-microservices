...
export const updateEarningPercentage= async(req:Request,res:Response,next:NextFunction)=>{
    try {
        ...
        await sendMessage("update-earning-percentage",randomKey,`${earning_percentage}`)
...
export const updateTermsAndConditions=async(req:Request,res:Response,next:NextFunction)=>{
        try {
            const {termsAndConditions,adminId} = req.body
            if(!termsAndConditions){
                throw new UserError("missing Terms and conditions !")
            }
            const response = await AdminService.updateTermsAndConditions(termsAndConditions,adminId)
            // notify users 
            const randomKey = getRandomKey()
...
export const updatePrivacyAndPolicy = async (req:Request,res:Response,next:NextFunction)=>{
    try {
        const {privacyAndPolicy,adminId} = req.body 
        if(!privacyAndPolicy){
            throw new UserError("missing privacy and policy !")
        }
        const response = await AdminService.updatePolicyAndPrivacy(privacyAndPolicy,adminId)
        const randomKey = getRandomKey()
...
export const publishAnAPI=async(req:Request,res:Response,next:NextFunction)=>{
    try {
        ...
        // notify users 
        const randomKey=getRandomKey()
        await sendMessage("publish-API",randomKey,`a new api ${api.name} has been launched`)
        return res.status(200).send(true)
    } catch (error) {
        next(error)
    }
...
export const getInactiveAPIS = async (req:Request,res:Response,next:NextFunction)=>{
    try {
