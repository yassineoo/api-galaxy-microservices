...
export const updateEarningPercentage = async (earningPercentage:number,adminId:number)=>{
    try {
...
export const scheduelEarningPercentage=async(earning_percentage:number,adminId:number)=>{
    try {
        // calculate the exacte time after 7 days
        const after7daysFromNow = dayJs().add(7,"day")

        // create a cron expression for that 
        const cronExpression=`${after7daysFromNow.second()} ${after7daysFromNow.minute()} ${after7daysFromNow.hour()} ${after7daysFromNow.date()} ${after7daysFromNow.month() + 1} *`
