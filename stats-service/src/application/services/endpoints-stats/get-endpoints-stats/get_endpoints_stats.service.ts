...
export async function get_stats(duration: Duration, endpoint_ids: ID[]) {
  ...
  const results = await Promise.all(
    periods.map(async (period) => getStatsInPeriod(period, endpoint_ids))
  );
...
async function getStatsInPeriod(period: Period, endpoint_ids: ID[]) {
  const stats = await Promise.all(
    endpoint_ids.map(async (endpoint_id) => {
...
async function getEndpointSuccessCallsInPeriod(
  period: Period,
  endpoint_id: ID
): Promise<number> {
...
async function getEndpointErrroCallsInPeriod(
  period: Period,
  endpoint_id: ID
): Promise<number> {
...
async function getEndpointCallsLatencyInPeriod(
  period: Period,
  endpoint_id: ID
): Promise<number> {
