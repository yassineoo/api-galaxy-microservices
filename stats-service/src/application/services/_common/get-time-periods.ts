...
export function getTimePeriods(duration: Duration): Period[] {
  ...
  switch (duration) {
    case "1h":
      ...
      keys = Array.from({ length: 12 }).map(
        (_, i) => `${60 - i * (60 / 12)} min ago`
      );
    ...
    case "24h":
      ...
      keys = Array.from({ length: 12 }).map((_, i) =>
        (12 - i) * 2 === 24 ? "1 day ago" : `${(12 - i) * 2} hour ago`
      );
      periods = getPeriods(12, realDuration, referenceDate, keys);
      break;
    ...
    case "90d":
      ...
      keys = Array.from({ length: 12 }).map((_, i) => {
        ...
        if ((i + 1) % 4 === 0)
          return `${3 - Math.floor((i + 1) / 4)} month${
            3 - Math.floor((i + 1) / 4) > 1 ? "s" : ""
          } ago`;
