...
exports.getTimePeriods = void 0;
const hour = 3600 * 1000;
const day = 24 * hour;
const week = 7 * day;
const month = 30 * day;
const year = 365 * day;
function getTimePeriods(duration) {
    let realDuration;
    ...
    switch (duration) {
        ...
        case "90d":
            ...
            referenceDate.setMonth(referenceDate.getMonth() - 3);
            keys = Array.from({ length: 12 }).map((_, i) => {
                if (3 - Math.floor((i + 1) / 4) === 0)
                    return "current week";
                if ((i + 1) % 4 === 0)
                    return `${3 - Math.floor((i + 1) / 4)} month${3 - Math.floor((i + 1) / 4) > 1 ? "s" : ""} ago`;
...
function getPeriods(cuts, duration, start, keys) {
    const periods = [];
